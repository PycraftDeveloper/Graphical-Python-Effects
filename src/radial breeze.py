...
bseed = getseed(random.randint(0, 10000))
k = 255/2
now_time = 0
start = time.perf_counter()
while registry.running:
    window_x, window_y = canvas.get_size()
    try:
        canvas.clear(0, 0, 0)
        ...
        for x in range(window_x, 0, -8):
            r = k*(1+generatekey((now_time+x)/2000, 0, rseed))
            g = k*(1+generatekey((now_time+x)/2000, 0, gseed))
