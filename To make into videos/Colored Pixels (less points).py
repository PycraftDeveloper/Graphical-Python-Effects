...
while True:
    ...
    for row in range(0, display.get_width(), 12):
        for column in range(0, display.get_height(), 12):
            r = row + 64*generatekey((row+x)/500, column/500, seed)
            c = column + 64*generatekey(row/500, (column+x)/500, seed)
